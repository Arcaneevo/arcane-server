  ...
  socket.on('boss:action', async ({ roomId, playerId, action, attackType, target }) => {
    ...
    if (room.currentTurnIndex === 0) {
      io.to('boss:'+roomId).emit('boss:state_update', {
        ...
        attackerId: playerId, attackType, attackTarget: target, dmg, crit: dmg > 15,
        bossTurn: true
      });
...
function doBossTurn(roomId) {
  ...
  // Esqueleto ataca si está vivo
  if (room.skeletonAlive) {
    const skTarget = alive[Math.floor(Math.random()*alive.length)];
    const skDmg = Math.floor(Math.random()*8) + 5; // 5-12 daño
    skTarget.hp = Math.max(0, skTarget.hp - skDmg);
    if (skTarget.hp <= 0) skTarget.isAlive = false;
    actionLog += ` · 🦴 Esqueleto atacó a ${skTarget.username}: -${skDmg}HP`;
  }
